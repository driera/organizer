...
- `npm run lint`: Runs ESLint on all `.ts` and `.tsx` files with strict settings.
- `npm test`: Runs type checking (`tsc --noEmit`) followed by Jest tests.
- `npm run test:watch`: Runs Jest in watch mode for interactive test development.
